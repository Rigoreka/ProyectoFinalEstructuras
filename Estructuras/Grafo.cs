    ...
    public partial class Grafo : Form
    {
        ...
        {         
            try
            {
                if (grafos == null)
                {
                    throw new Exception("Ingrese un dato para poder mostrar sus nodos adyacentes");
                }

                if (v < 0 || v >= grafos.vertice)
                {
                    throw new Exception("El dato ingresado supera el vertice definido con anterioridad");
                }

                txtImprimir.AppendText($"\nLos nodos adyacentes de {v} son \n");
            ...
            {
                MessageBox.Show($"{e.Message}, El dato ingresado fue: {v}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void txtImprimir_KeyPress(object sender, KeyPressEventArgs e)
        {
            e.Handled = true;
        }
    }
